import { Swiper, SwiperSlide } from "swiper/react";
import { testimonialsSlider } from "../sliderProps";
import React, { useEffect, useState } from "react";
const TestimonialSlider = () => {
  const url =
    "https://portfolio-backend-30mp.onrender.com/api/v1/get/user/65b3a22c01d900e96c4219ae";
  const [data, setData] = useState([]);
  // Including items which are enabled 
  const fetchInfo = () => {
    return fetch(url)
      .then((res) => res.json())
      .then((d) => setData(d.user.testimonials.filter(item=>item.enabled))); 
  };

  useEffect(() => {
    fetchInfo();
  }, []);
  return (
    <div className="m-testimonials">
      <Swiper
        {...testimonialsSlider}
        className="swiper-container js-testimonials"
      >
        <div className="swiper-wrapper">
          {data &&
            data.map(({ image, name, review, position }, index) => (
              <SwiperSlide className="swiper-slide">
                <div className="testimonials-item">
                  <div className="text">
                    <div className="icon">
                      <svg
                        width={58}
                        height={44}
                        xmlns={image.public_id}
                        version="1.1"
                        xmlnsXlink={image.url}
                        viewBox="0 0 58 44"
                      >
                        <defs />
                        <g>
                          <g>
                            <path
                              d="M24.15697,5.25606c-5.87975,2.24304 -9.65698,8.89397 -9.65698,14.29844h9.62551v24.44363h-24.12552v-18.06641c0,-13.94272 9.05766,-23.39363 21.74995,-25.93231zM57.98788,5.25606c-5.87734,2.24304 -9.65458,8.89397 -9.65458,14.29844h9.6666v24.44363h-24.12552v-18.06641c0,-13.94272 9.01657,-23.39363 21.70887,-25.93231zM54.21305,2.82419c-11.10706,2.9459 -17.92202,11.65104 -17.92202,23.10753v15.62309h19.29222v-19.55605h-9.6666v-2.44427c0,-5.29194 3.09334,-12.05826 8.87878,-15.45906zM20.37974,2.82133v0l0.58239,1.27411c-5.78556,3.4008 -8.8789,10.16712 -8.8789,15.45906v2.44427v0h9.62563v0v19.55605v0h-19.29222v0v-15.62309c0,-11.45458 6.83189,-20.15972 17.9631,-23.11039"
                              fill="#ffffff"
                              fillOpacity={1}
                            />
                          </g>
                        </g>
                      </svg>
                    </div>
                    <p>{review}</p>
                  </div>
                  <div className="image">
                    <img src={image.url} alt={name} />
                  </div>
                  <div className="info">
                    <h3 className="name">{name}</h3>
                    <div className="author">{position}</div>
                  </div>
                </div>
              </SwiperSlide>
            ))}
          {/* Not used , but kept for future use if we need to add */}
          {/* <SwiperSlide className="swiper-slide">
           <div
             className="testimonials-item"
           >
             <div className="text">
               <div className="icon">
                 <svg
                   width={58}
                   height={44}
                   xmlns="http://www.w3.org/2000/svg"
                   version="1.1"
                   xmlnsXlink="http://www.w3.org/1999/xlink"
                   viewBox="0 0 58 44"
                 >
                   <defs />
                   <g>
                     <g>
                       <path
                         d="M24.15697,5.25606c-5.87975,2.24304 -9.65698,8.89397 -9.65698,14.29844h9.62551v24.44363h-24.12552v-18.06641c0,-13.94272 9.05766,-23.39363 21.74995,-25.93231zM57.98788,5.25606c-5.87734,2.24304 -9.65458,8.89397 -9.65458,14.29844h9.6666v24.44363h-24.12552v-18.06641c0,-13.94272 9.01657,-23.39363 21.70887,-25.93231zM54.21305,2.82419c-11.10706,2.9459 -17.92202,11.65104 -17.92202,23.10753v15.62309h19.29222v-19.55605h-9.6666v-2.44427c0,-5.29194 3.09334,-12.05826 8.87878,-15.45906zM20.37974,2.82133v0l0.58239,1.27411c-5.78556,3.4008 -8.8789,10.16712 -8.8789,15.45906v2.44427v0h9.62563v0v19.55605v0h-19.29222v0v-15.62309c0,-11.45458 6.83189,-20.15972 17.9631,-23.11039"
                         fill="#ffffff"
                         fillOpacity={1}
                       />
                     </g>
                   </g>
                 </svg>
               </div>
               
               <p>
                 {data[1]?review:''}
               </p>
             </div>
             <div className="image">
               <img src="assets/images/team3.jpg" alt="Jennifer Smith" />
             </div>
             <div className="info">
               <h3 className="name">{name}</h3>
               <div className="author">CEO &amp; Founder</div>
             </div>
           </div>
         </SwiperSlide>
        
         <SwiperSlide className="swiper-slide">
           <div
             className="testimonials-item"
           >
             <div className="text">
               <div className="icon">
                 <svg
                   width={58}
                   height={44}
                   xmlns="http://www.w3.org/2000/svg"
                   version="1.1"
                   xmlnsXlink="http://www.w3.org/1999/xlink"
                   viewBox="0 0 58 44"
                 >
                   <defs />
                   <g>
                     <g>
                       <path
                         d="M24.15697,5.25606c-5.87975,2.24304 -9.65698,8.89397 -9.65698,14.29844h9.62551v24.44363h-24.12552v-18.06641c0,-13.94272 9.05766,-23.39363 21.74995,-25.93231zM57.98788,5.25606c-5.87734,2.24304 -9.65458,8.89397 -9.65458,14.29844h9.6666v24.44363h-24.12552v-18.06641c0,-13.94272 9.01657,-23.39363 21.70887,-25.93231zM54.21305,2.82419c-11.10706,2.9459 -17.92202,11.65104 -17.92202,23.10753v15.62309h19.29222v-19.55605h-9.6666v-2.44427c0,-5.29194 3.09334,-12.05826 8.87878,-15.45906zM20.37974,2.82133v0l0.58239,1.27411c-5.78556,3.4008 -8.8789,10.16712 -8.8789,15.45906v2.44427v0h9.62563v0v19.55605v0h-19.29222v0v-15.62309c0,-11.45458 6.83189,-20.15972 17.9631,-23.11039"
                         fill="#ffffff"
                         fillOpacity={1}
                       />
                     </g>
                   </g>
                 </svg>
               </div>
               <p>
                 Sed vel mattis dolor. Vestibulum gravida, dolor vel dapibus
                 bibendum, velit enim finibus mauris, in volutpat arcu magna ac
                 metus. Sed condimentum, erat vel sodales pharetra, ligula
                 massa maximus dolor
               </p>
             </div>
             <div className="image">
               <img src="assets/images/team3.jpg" alt="Jennifer Smith" />
             </div>
             <div className="info">
               <h3 className="name">Jennifer Smith</h3>
               <div className="author">CEO &amp; Founder</div>
             </div>
           </div>
         </SwiperSlide> */}
        </div>
        <div className="swiper-button-prev">
          <svg
            width={13}
            height={25}
            xmlns="http://www.w3.org/2000/svg"
            version="1.1"
            xmlnsXlink="http://www.w3.org/1999/xlink"
            viewBox="0 0 13 25"
          >
            <defs />
            <g>
              <g>
                <path
                  d="M0.27409,11.8138l11.13338,-11.53183c0.36484,-0.37766 0.95535,-0.37766 1.3195,0.00191c0.36392,0.37861 0.36301,0.99277 -0.00183,1.37043l-10.47066,10.84518v0l10.471,10.84423c0.36484,0.37861 0.36575,0.99087 0.00195,1.37043c-0.18253,0.18883 -0.42171,0.28515 -0.6609,0.28515c-0.23849,0 -0.47676,-0.09537 -0.65895,-0.28229l-11.1335,-11.53183c-0.17567,-0.18215 -0.2742,-0.4282 -0.2742,-0.68569c0,-0.25749 0.09888,-0.50449 0.2742,-0.68569z"
                  fill="#ffffff"
                  fillOpacity={1}
                />
              </g>
            </g>
          </svg>
        </div>
        <div className="swiper-button-next">
          <svg
            width={13}
            height={25}
            xmlns="http://www.w3.org/2000/svg"
            version="1.1"
            xmlnsXlink="http://www.w3.org/1999/xlink"
            viewBox="0 0 13 25"
          >
            <defs />
            <g>
              <g>
                <path
                  d="M0.27409,11.8138l11.13338,-11.53183c0.36484,-0.37766 0.95535,-0.37766 1.3195,0.00191c0.36392,0.37861 0.36301,0.99277 -0.00183,1.37043l-10.47066,10.84518v0l10.471,10.84423c0.36484,0.37861 0.36575,0.99087 0.00195,1.37043c-0.18253,0.18883 -0.42171,0.28515 -0.6609,0.28515c-0.23849,0 -0.47676,-0.09537 -0.65895,-0.28229l-11.1335,-11.53183c-0.17567,-0.18215 -0.2742,-0.4282 -0.2742,-0.68569c0,-0.25749 0.09888,-0.50449 0.2742,-0.68569z"
                  fill="#ffffff"
                  fillOpacity={1}
                />
              </g>
            </g>
          </svg>
        </div>
      </Swiper>
    </div>
  );
};
export default TestimonialSlider;
